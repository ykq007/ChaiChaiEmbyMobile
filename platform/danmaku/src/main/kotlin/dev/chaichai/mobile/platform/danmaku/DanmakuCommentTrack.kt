...
class DanmakuCommentTrack(
    ...
    fun snapshotAt(positionTicks: Long, isPaused: Boolean, speed: Float): DanmakuOverlaySnapshot {
        ...
        val scrollLaneFreeAt = LongArray(maxLanes) { Long.MIN_VALUE }
        var pinnedLane = 0
        val visible = ArrayList<DanmakuVisibleComment>(maxVisible)
        ...
        for (comment in ordered) {
            ...
            val lane: Int = when (comment.position) {
                DanmakuPosition.Scroll -> {
                    val chosen = comment.laneHint?.let { it % maxLanes }
                        ?.takeIf { scrollLaneFreeAt[it] <= comment.timeTicks }
                        ?: (0 until maxLanes).firstOrNull { scrollLaneFreeAt[it] <= comment.timeTicks }
                ...
                DanmakuPosition.Top, DanmakuPosition.Bottom -> {
                    val chosen = pinnedLane % maxLanes
                    pinnedLane++
                    chosen
                }
            }
